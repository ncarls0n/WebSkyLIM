...
def add_variable_log_normal_scatter( data, z_halo, interp_table_file=None,
        pwr=1.0, seed=None ):
    """
    """
    # default interpollation table for line luminosity model parameters
    if interp_table_file is None:
        interp_table_file = ('/home/njcarlson/Repositories/CITA_LIM/limla'+
                'm_mocker/tables/FIRE-forged_CO.txt')
    ...
    # For now, we just do a linear interpollation between model parameters 
    sigma_dex = np.interp( z_halo, interp_table[:,0], interp_table[:,5] )
    
    # Calculate linear sigma and mean from log-normal
    sigma = sigma_dex * np.log(10)
    mu    = -0.5*sigma**2
    # Note that the standard deviation in log space is different from the
    ...
    # 
    randscaling   = np.random.lognormal(  mu, sigma )
    datascattered = np.where( data > 0, data*randscaling**pwr, data )

    return datascattered
