...
class LimLam(LineObs):
    ...
    def halo_info(self):
        ...
        # for other .npz catalogues
        elif self.catalogue_file.endswith('.npz'):
            return np.load(self.catalogue_file,allow_pickle=True)
        
        # For peak patch formatted output
        else:

            # Import the Peak Patch module
            import os, psutil
            ...
            r = PeakPatch( os.path.abspath( os.path.dirname(self.catalogue_file)+'/..' ) )

            # Set physical constants
            H_0   = r.h * 100 # km/s/Mpc
            G     = 4.300917270036279e-09 # km^2 Mpc Msol^-1 s^-2
            rho_c = 3*H_0**2 / (8*np.pi*G) # Msol Mpc^-3
            rho_m = rho_c * ( r.Omx + r.OmB ) # Msol Mpc^-3

            # Set the header for the catalogue
            cosmo_header = { 'Omega_M' : r.Omx+r.OmB , 'Omega_B' : r.OmB , 'Omega_L' : r.Omvac ,
                    'h' : r.h , 'ns' : r.ns , 'sigma8' : r.sigma8 }

            # # Get the number of parallel cores available
            # N_proc = psutil.cpu_count( logical=True ) # logical=False to only count physical cores

            # Generate interpolation table for redshift to comoving distance calculation
            z_chi_tab = pkp.z_of_r_comoving_table( zmin=1.0e-3, zmax=3402.0, dlog10z=6.538e-3,
                    Omega_r0=0.0, Omega_m0=r.Omx+r.OmB, Omega_k0=0.0, Omega_Lambda=r.Omvac,
                    H_0=r.h*100, c=299792458.0 )
            
            # Read in the halo catalogue
            x,y,z,dx,dy,dz,M,redshift,zform,zcos = pkp.add_halos_pksc_to_npz(
                    catalogue_file=self.catalogue_file , mass_cutoff=1.5*r.cellsize , rho_m=rho_m ,
                    z_range=None , z_chi_tab=z_chi_tab )

            # Return the catalogue as a dict
            return { 'cosmo_header' : np.array( cosmo_header, dtype=object ),
                     'x'            : x,
                     'y'            : y,
                     'z'            : z,
                     'vx'           : dx,
            ...
            # # Read the Peak Patch catalogue
            # catalogue = pkp.add_halos_pksc_to_npz( catalogue_file=self.catalogue_file ,
            #         mass_cutoff=1.5*r.cellsize , rho_m=rho_m , z_range=None , z_chi_tab=z_chi_tab )
            # 
            # return { 'cosmo_header' : np.array( cosmo_header, dtype=object ),
            #          'x'            : catalogue[0],
            #          'y'            : catalogue[1],
    ...
    def noise_added_map(self): # added by Clara
        ...
        # Noise temperature T_{sys} in microkelvin
        noise_sigma = self.sigma_N

        # Noise mean set to 0
        noise_mean  = 0.

        # The number of samples of the Gaussian random variable representing noise
        noise_samples = map.shape

        # If noise is measured as noise-equivalent flux density (NEFD), T_{sys} converted to Jy/sr
        if self.do_Jysr:
            noise_sigma = noise_sigma.to( u.Jy/u.sr, 
                                          equivalencies = u.brightness_temperature(self.nuObs) 
                                        )
        
        # Noise standard deviation is a unitless float
        noise_sigma = noise_sigma.value

        # Produce Gaussian white noise map
        noise_map = np.random.normal( noise_mean, noise_sigma, noise_samples )
        
        # Add noise map to Gaussian filtered map
